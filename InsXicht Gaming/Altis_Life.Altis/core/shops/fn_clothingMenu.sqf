...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb"] && playerSide != civilian) exitWith {hint "Du kannst diesen Shop nur als Zivilist benutzen."; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Du kannst diesen Shop nur mit Rebellen Lizenz benutzen."; closeDialog 0;};
if((_this select 3) == "med" && playerSide != independent) exitWith {hint "Du kannst diesen Shop nur als Sani Beamter benutzen"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Du kannst diesen Shop nur als Polizist benutzen."; closeDialog 0;};
...
{
	if(!(missionNamespace getVariable (_var select 0))) exitWith {hint format["Du brauchst ein %1 um dies zu Kaufen!",[_var select 0] call life_fnc_varToStr]; closeDialog 0;};
};
