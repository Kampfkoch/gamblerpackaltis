...
if(!([_amount] call fnc_isnumber)) exitWith {hint "Du musst eine gueltige Zahl eingeben";};
_amount = parseNumber (_amount);
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "Du hast nicht so viele Items zu verkaufen!"};
...
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	...
	hint format["Du verkaufst %1 %2 fuer $%3",_amount,_name,[_price] call life_fnc_numberText];
